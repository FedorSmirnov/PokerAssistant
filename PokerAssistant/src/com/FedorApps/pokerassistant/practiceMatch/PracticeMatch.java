...
import com.FedorApps.pokerassistant.pokerSession.BetAmountPick;
import com.FedorApps.pokerassistant.pokerSession.CircleList;
import com.FedorApps.pokerassistant.pokerSession.OtherPlayer;
import com.FedorApps.pokerassistant.pokerSession.Player;
import com.FedorApps.pokerassistant.pokerSession.PsMenuActivity;
import com.FedorApps.pokerassistant.pokerSession.PsMenuBlindActivity;
import com.FedorApps.pokerassistant.pokerSession.PsMenuPlayerIn;
import com.FedorApps.pokerassistant.pokerSession.PsMenuPlayerOut;
import com.FedorApps.pokerassistant.pokerSession.You;
...
public class PracticeMatch extends Activity {
	...
	private int[] copydeck = new int[52];
	...
	public static final int MSG_RESULT = 10000;
	public static final int MSG_VIRT_RESULT = 20000;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 1; i < playerNum; i++) {
			...
			VirtualPlayer player = new VirtualPlayer(startAmount, name, i, this);
		...
		int num = random.nextInt(playerButtons.size());

		Player dealer = playerButtons.get(num);
		...
		// Falls ein anderer Spieler dran ist, so lange weitermachen, bis you
		// dran ist.
		// runGameLoop();
	...
	private void dealCards() {
		...
		for (int i = 0; i < sessionPlayers.size(); i++) {
			...
			while (num == 0) {
				rand = random.nextInt(copydeck.length);
			...
			Card card = new Card(color, value, this);

			if (sessionPlayers.get(i) instanceof You) {
				You player = (You) sessionPlayers.get(i);
				player.setCard1(card);
			} else {
				VirtualPlayer player = (VirtualPlayer) sessionPlayers.get(i);
				player.getHand().setCard1(card);
			}
			...
			Card card2 = new Card(color, value, this);
	...
	private void runGameLoop() {
		...
		if (onTurnNum != you) {
			...
			// Virtueller Spieler dran: er muss seine Quoten abfragen und
			// handeln. Anschließend muss die Quote des Spielers errechnet
			// werden.
	...
	private void reactToOdd(int odd) {

		// Toast.makeText(getApplicationContext(), "Quote" + odd,
		// Toast.LENGTH_SHORT).show();

		int limit = potWithBets * odd / 100;

		if (limit < currentBet - onTurnNum.getBet()) {
			// folden
			onClickFoldButton();
		} else {
			// callen
	...
	class IncomingHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_VIRT_RESULT:

				int oddVirt = msg.getData().getInt(PreferenceContainer.oddKey);
	...
	private void loadStartValues() {
		...
		difficulty = PreferenceContainer.loadInt(
	...
	private void callVirtCalc() {

		if (!serviceBound) {
			return;
		}

		Message msg = Message.obtain(null, OddCalculatorService.MSG_VIRT_CALC,
				0, 0);
		Bundle data = new Bundle();

		// Alle 7 KARTEN werden als Integer ins Bündel gepackt.

		if (onTurnNum instanceof VirtualPlayer) {

			String colorKey = getKeyStringColor(1);
			String valueKey = getKeyStringValue(1);

			int color = ((VirtualPlayer) onTurnNum).getHand().getCard1()
					.getColor();
			int value = ((VirtualPlayer) onTurnNum).getHand().getCard1()
					.getValue();
			...
			colorKey = getKeyStringColor(2);
			valueKey = getKeyStringValue(2);

			color = ((VirtualPlayer) onTurnNum).getHand().getCard2().getColor();
			value = ((VirtualPlayer) onTurnNum).getHand().getCard2().getValue();
		...
		// Der Flop wird gepackt

		if (flop.getCard1() != null) {

			String colorKey = getKeyStringColor(3);
			String valueKey = getKeyStringValue(3);

			int color = flop.getCard1().getColor();
			int value = flop.getCard1().getValue();
			...
			colorKey = getKeyStringColor(4);
			valueKey = getKeyStringValue(4);
			...
			colorKey = getKeyStringColor(5);
			valueKey = getKeyStringValue(5);
		...
		if (turn.getCard() != null) {
			...
			String colorKey = getKeyStringColor(6);
			String valueKey = getKeyStringValue(6);

			int color = turn.getCard().getColor();
			int value = turn.getCard().getValue();
		...
		// Der River wird gepackt

		if (river.getCard() != null) {

			String colorKey = getKeyStringColor(7);
			String valueKey = getKeyStringValue(7);

			int color = river.getCard().getColor();
			int value = river.getCard().getValue();
	...
	private void callCalc() {
		...
		// Die Karten von dem Spieler werden gepackt

		String colorKey = getKeyStringColor(1);
		String valueKey = getKeyStringValue(1);

		int color = you.getHand().getCard1().getColor();
		int value = you.getHand().getCard1().getValue();
		...
		color = you.getHand().getCard2().getColor();
		value = you.getHand().getCard2().getValue();
		...
		if (flop.getCard1() != null) {
			...
			colorKey = getKeyStringColor(3);
			valueKey = getKeyStringValue(3);
		...
		if (turn.getCard() != null) {
			...
			colorKey = getKeyStringColor(6);
			valueKey = getKeyStringValue(6);
		...
		if (river.getCard() != null) {
			...
			colorKey = getKeyStringColor(7);
			valueKey = getKeyStringValue(7);
	...
	private void setSessionOnClickListeners() {
		...
		for (int i = 0; i < sessionPlayers.size(); i++) {
			...
			if (!(player2 instanceof You)) {
				...
				player.getStatBut().setOnClickListener(
	...
	private void transferPhase() {
		...
		Toast.makeText(getApplicationContext(), "Transfer Phase",
				Toast.LENGTH_SHORT).show();
	...
	private void dealFlop() {
		...
		Card card3 = new Card(color, value, this);
		flop.setCard3(card3);

		setButtonColor(flopBox3, card.getColor());
		setButtonText(flopBox3, card);

		setButtonColor(flopBox2, card2.getColor());
		setButtonText(flopBox2, card2);

		setButtonColor(flopBox1, card3.getColor());
		setButtonText(flopBox1, card3);
	...
	private void dealTurn() {
		...
		setButtonColor(turnBox, card.getColor());
		setButtonText(turnBox, card);
	...
	private void dealRiver() {
		...
		setButtonColor(riverBox, card.getColor());
		setButtonText(riverBox, card);
	...
	private int getLayoutId(int playerNum) {
		...
		case 2:
			id = R.layout.activity_practice_match_2pl;
			break;
